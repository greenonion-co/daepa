 ...
 *
 * - BottomSheet: 7일간 다시 안 보기 (localStorage)
 * - KakaoInAppGuide: 세션 동안 다시 안 보기 (sessionStorage)
 */

const BOTTOM_SHEET_KEY = "appPrompt.bottomSheet.dismissedUntil";
...
export function isBottomSheetDismissed(): boolean {
  ...
  try {
    const v = localStorage.getItem(BOTTOM_SHEET_KEY);
    if (!v) return false;
...
export function dismissBottomSheet(): void {
  ...
  try {
    localStorage.setItem(BOTTOM_SHEET_KEY, String(Date.now() + SEVEN_DAYS_MS));
  } catch {

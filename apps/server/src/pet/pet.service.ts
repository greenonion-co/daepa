...
import { CsvPreviewResponseDto, CreatePetDto, PetDto } from './pet.dto';
import {
  PET_GROWTH,
  PET_SEX,
...
export class PetService {
  ...
  async previewCsvFile(file: Express.Multer.File) {
    const records = await this.parseCsvFile(file.buffer);
    const results = {
      uploadedCount: 0,
      failedCount: 0,
      errors: [] as string[],
      previewData: [] as PetDto[],
    };
    const validRecords: PetDto[] = [];
    ...
    for (let i = 0; i < records.length; i++) {
      try {
        ...
        if (validationError) {
          ...
          results.uploadedCount++;
          validRecords.push(plainToInstance(PetDto, records[i]));
        }
      } catch (error) {
        results.failedCount++;
        const errorMessage =
